...
def main():
    ...
    parser.add_argument('--graph', type=str, default="graph",
                        help="Use graph of hypergraph in graph networks, options: [graph, hypergraph]")
    ...
    parser.add_argument('--sparse_flag', type=bool, default=False,
                        help='Whether to apply logsparse mask for LogSparse Transformer')
    parser.add_argument('--win_len', type=int, default=6,
                        help='Local attention length for LogSparse Transformer or length window of Adarnn')
    ...
    parser.add_argument('--v_conv', type=int, default=0,
                        help='Whether to apply ConvAttn for values (in addition to K/Q for LogSparseAttn')
    parser.add_argument('--win_size', type=int, default=2,
                        help='Merging every window size of adjacent segments in Crossformer')
    ...
    parser.add_argument('--flat', type=str, default="False",
                        help="Whether the sequence data should be transformed into flatten form")
    ...
    parser.add_argument('--min_split', type=int, default=10000,
                        help="Minimum samples required for a split in tree model")
    ...
    parser.add_argument('--trans_loss', type=str, default='adv',
                        help="Type of loss to calculate losses between domains")
    ...
    parser.add_argument('--bottleneck_width', type=int, default=64,
                        help="bottleneck width of bottleneck network in Adarnn")
    ...
    for ii in range(args.itr):
        ...
        for year in range(args.train_start, args.train_start + args.test_size):
            ...
            train_start = date_ls[
                max(bisect.bisect_left(date_ls, str(args.train_start) + "-01-01") - args.seq_len, 0)]
            ...
            if args.full:
                ...
                for j in tqdm(range(len(test_dates))):
                    date = test_dates[j]
                    df_today = df_test[df_test['date'] == date].loc[:,
                               ['code', 'pred', 'date', 'label', 'DlyPrc', 'ShrOut', 'DlyVol', 'SecurityEndDt']]
