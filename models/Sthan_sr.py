...
class Attention(torch.nn.Module):
    def __init__(self, dimensions, query_len):
        ...
        self.gamma = torch.nn.Parameter(torch.FloatTensor(1,1,1)) 
        self.epsilon = torch.nn.Parameter(torch.FloatTensor(1,1,1))
        self.gamma.requires_grad = True
        self.epsilon.requires_grad = True

        self.delta_t = torch.flip(torch.arange(1, query_len + 1), [0]).float()
    ...
    def forward(self, query, key, value):
        ...
        #delta
        self.delta_t = self.delta_t.reshape(1, -1, 1).repeat(lam.shape[0], 1, lam.shape[2])
        # e^ -gamma delta
        decay = torch.exp(-1 * self.gamma * self.delta_t)
...
class Model(torch.nn.Module):
    def __init__(self, args):
        ...
        self.attention = Attention(self.out_dim, args.seq_len - 1)
        self.layers = torch.nn.ModuleList()
        for i in range(self.args.e_layers):
            self.layers.append(nn.HypergraphConv(self.out_dim, self.out_dim, use_attention=True, heads=self.args.head, concat=False, negative_slope=self.args.negative_slope, dropout=self.args.dropout, bias=True))
    ...
    def forward(self, input_feature, inci_mat, edge):
        ...
        for layer in self.layers:
            output = self.activation(layer(output, edge, hyperedge_attr=edge_attr), 0.2)
